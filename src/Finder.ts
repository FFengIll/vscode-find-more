  ...
  if (1) {
    ...
    switch (markerType) {
      case "background":
        ...
        rules["color"] = /* getColorContrast(color) === */ "dark"
          ? "#111"
          : "#fff"
        break
      case "underline":
...
export default class Finder {
  ...
  private useDiagnostics: Boolean | undefined
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    // get ripgrep
    this.engine =
      vscode.workspace.getConfiguration().get<string>("find.engine") || ""
    var option_str: string =
      vscode.workspace.getConfiguration().get<string>("find.options") || ""
    this.engine_options = option_str.split(" ")

    // use  diagnostics
    this.useDiagnostics = vscode.workspace
      .getConfiguration()
      .get<boolean>("find.diagnostics")
  ...
  public find(target: string) {
    ...
    var ranges = []
    var lines = []
    var filepath = editor.document.uri.fsPath.toString()
